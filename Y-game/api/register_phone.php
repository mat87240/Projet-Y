...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["nom"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["telephone"])) {
        $nom = trim($_POST["nom"]);
        ...
        if (!empty($nom) && !empty($email) && !empty($password) && !empty($telephone)) {
            ...
            // Insérer l'utilisateur dans la base de données avec le téléphone
            $stmt = $conn->prepare("INSERT INTO users (nom, email, password, telephone) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $nom, $email, $hashedPassword, $telephone);
